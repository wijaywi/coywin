...
text = open('best_cmd.txt', 'r', encoding='utf-8').read()
data = json.loads(text)
cmd = data['args']['CommandLine']
payload = cmd.split("Set-Content full_merkle.ps1 '\n")[1].split("\n'\nCopy-Item")[0]

payload = payload.replace('meta.enable_constant(rc_b[0]);', 'meta.enable_constant(rc_b[0]);\n        for col in state.iter() {\n            meta.enable_equality(*col);\n        }\n        meta.enable_equality(partial_sbox);')
...
payload = payload.replace('meta.query_fixed(fixed_width, Rotation::cur())', 'meta.query_fixed(fixed_width)')
payload = payload.replace('meta.query_fixed(fixed_height, Rotation::cur())', 'meta.query_fixed(fixed_height)')
...
with open('coywin-zksteg/src/full_merkle.rs', 'w', encoding='utf-8') as f:
    f.write(payload)
